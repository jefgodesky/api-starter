...
A starting point for RESTful API projects that
...
**Work in Progress.** The rest of this README lays out
the vision for this project, but it isn’t working yet!
...
that 10th time, that usually means that I
haven’t thought about it hard enough and it
turns out that API-first is _still_ the way
to go). This means that, at the start of most
projects, I keep doing the same thing over
and over again: setting up Docker, the
database, the continuous deployment
...
In the same directory is `perms.yml`,
...
The `default` setting in `perms.yml` lists
...
and returns them to the user. That means that
throughout your own middlewares, you can
throw errors using `HTTPException` from
`hono/http-exception`. Take a look at the
`requireClient` middleware (found in
`middlewares/require/client.ts`) for an
example of usage.
...
const requireClient: Middleware = async (c, next) => {
  if (!c.client) throw new HTTPException(401, localize('authentication_required'))
  await next()
}
...
`localize` is a utility method that
...
If there’s a problem with an integration test,
it _might_ be something that’s only
...
* In each of the Docker Compose configuration files (found
  in `env/*.yml`), you’ll want to update the name to
