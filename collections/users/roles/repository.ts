...
export default class RoleRepository {
  ...
  async has (uid: string, role: Role): Promise<boolean | null> {
    if (!await this.userExists(uid)) return null
  ...
  async grant (uid: string, role: Role): Promise<boolean | null> {
    if (!await this.userExists(uid)) return null
  ...
  async revoke (uid: string, role: Role): Promise<boolean | null> {
    if (!await this.userExists(uid)) return null
